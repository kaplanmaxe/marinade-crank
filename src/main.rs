...
fn get_client(keypair: &String, cluster: &str) -> Result<RpcMarinade<Rc<solana_sdk::signature::Keypair>>> {
    let client = Client::new_with_options(
        Cluster::from_str(cluster).unwrap(),
        Rc::new(read_keypair_file(keypair).unwrap()), 
        CommitmentConfig::finalized(),
    );
    Ok(marinade_client_rs::marinade::rpc_marinade::RpcMarinade::new(
        &client, 
...
async fn send_transaction(validator_vote: String, validator_index: u32, keypair: &String, simulate: &bool, cluster: &String, compute_unit_price: &Option<u64>) {
    let rpc_url = cluster;
    ...
    .unwrap()
    .instruction(ComputeBudgetInstruction::set_compute_unit_price(match compute_unit_price {
        Some(price) => *price,
        None => 0
    }))
...
async fn main() {
    ...
    let rpc_client = RpcClient::new_with_commitment(args.cluster.to_string(), CommitmentConfig::finalized());
        ...
        Ok((validators, _count)) => {
            for (index, validator) in validators.iter().enumerate() {
                if validator.validator_account.to_string() == validator_vote {
                    let reserve_balance = client.client.get_account(&Pubkey::from_str(RESERVE_BALANCE).unwrap()).unwrap().lamports;
                    ...
                    let validator_stake_target = client.state.validator_system.validator_stake_target(validator, total_stake_target).unwrap() / 1000000000;
                    let validator_list_data = rpc_client.get_account_data(&Pubkey::from_str(VALIDATOR_LIST).unwrap()).await;
                    
                    validator_index = index as u32;
                    let validator = client.state.validator_system.get_checked(
                    ...
                    println!("Attempting to stake {} with {} SOL", validator_vote, (stake_target as f64));
                }
            }
        }
        Err(err) => { eprintln!("{}", err) }
    }
    send_transaction(validator_vote, validator_index, &args.keypair, &args.simulate, &args.cluster, &args.compute_unit_price).await;    
}